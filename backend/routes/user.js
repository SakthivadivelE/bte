                    ...
                    if (err) {
                     ...
                     con.query("insert into expired_data SET ?", data, (err, result) => {
                       if (err) {
                        ...
                        res.json({ token: jwt.sign({ email: user[0].email, username: user[0].username, id: user[0].id }, 'RESTFULAPIs', { expiresIn: '1h' }),user });
                       }
                     });
...
router.get('/listRejectedPosts', verifytoken, (req, res) => {
  con.query("select * from posts WHERE isRejected = 1",(err, result) => {
    if (err) {
      res.send({ error: err.message });
    } else {
      res.send(result);
